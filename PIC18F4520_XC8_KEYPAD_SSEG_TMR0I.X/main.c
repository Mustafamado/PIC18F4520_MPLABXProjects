/**
 * MPLABX IDE project example for KEYPAD SSEG TMR0I
 * 
 * @author   Burak Enez
...
int key_value = 0; // Value that will be shown on ssegs
...
void main(void) {
    ...
    while(1) {
        key_value = KEYPAD_ReadWaitUntilRelease(); // Read value from KEYPAD but program stuck in loop until KEY is released
    }
}
...
void interrupt ISR(void) {
    ...
    if(INTCONbits.TMR0IF==1) { // if Timer0 Interrupt is occurred
        ...
        if(timer_flag == 0) { // if it is first 10ms
            SSEG_Print(1,key_value%10); // Shows ones of key value at first sseg
            timer_flag = 1;
        }
        else { // if it is second 10ms
            SSEG_Print(2,key_value/10); // Shows tens of key value at second sseg
            timer_flag = 0;
        }
    }
}
